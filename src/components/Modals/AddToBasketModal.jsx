...
export default function AddToBasketModal({
  ...
  if (item.price12oz != null)
    availableSizes.push({ label: "12oz", price: Number(item.price12oz) });
  if (item.price16oz != null)
    availableSizes.push({ label: "16oz", price: Number(item.price16oz) });
  if (item.pricesingle != null)
    availableSizes.push({ label: "Single", price: Number(item.pricesingle) });

  const [qty, setQty] = useState(1);
  const [size, setSize] = useState(
    availableSizes.length ? availableSizes[0].label : ""
  );
  ...
  const calcTotal = () =>
    (getPrice() * qty).toLocaleString("en-PH", {
      style: "currency",
      currency: "PHP",
    });
  ...
  return (
    ...
    >
      ...
      <div className="d-flex justify-content-between gap-2">
        ...
        <button className="btn btn-warning w-50" onClick={handleAdd}>
          Add to Basket
        </button>
      </div>
    </Modal>
